...
class RemoteSync:
    ...
    async def rsync(cls, src: PathModel, dst: PathModel, *, options: list = []) -> str:
        ...
        """
        if src.protocol not in ["local_fs", "ssh"]:
            raise RemoteSyncError(f"Unsupported protocol {src.protocol} for source path")

        if dst.protocol not in ["local_fs", "ssh"]:
            raise RemoteSyncError(f"Unsupported protocol {dst.protocol} for destination path")

        if src.protocol == "ssh" and dst.protocol == "ssh":
        ...
        if src.protocol == "ssh":
            ...
            if src.host.user is not None:
                src_path = f"{src.host.user}@{src_path}"
            remote = src.host
        elif dst.protocol == "ssh":
            dst_path = f"{dst.host.host}:{dst_path}"
            if dst.host.user is not None:
                dst_path = f"{dst.host.user}@{dst_path}"
            remote = dst.host
    ...
    async def scp(cls, src: PathModel, dst: PathModel) -> str:
        ...
        if src.protocol == "local_fs" and dst.protocol == "local_fs":
